      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-12">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 animate-fade-in-up">
          <span className="inline-flex items-center gap-2 text-sm font-semibold tracking-[0.15em] uppercase text-[var(--gold-700)] bg-[var(--gold-50)] px-5 py-2.5 rounded-full border border-[var(--gold-200)] mb-6">
            <Star className="w-4 h-4 text-[var(--gold-500)]" />
            {t('eyebrow')}
          </span>
          <h2 className="text-3xl lg:text-5xl font-bold leading-[1.15] tracking-tight text-gray-900">
            {t('title')}
          </h2>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map(({key}, idx) => (
            <div
              key={key}
              className={`bg-white border border-gray-200 rounded-xl p-8 relative group card-hover animate-fade-in-up`}
              style={{animationDelay: `${200 + idx * 150}ms`}}
            >
              {/* Quote icon */}
              <div className="absolute top-6 right-6 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
                <Quote className="w-10 h-10 text-[var(--purple-400)]" />
              </div>
